...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    prob = 1
    for person in people:
        gene_count = getgenecount(person, one_gene, two_genes)
        has_trait = person in have_trait
        mother = people[person]["mother"]
        father = people[person]["father"]
        if mother == None and father == None:
            prob = PROBS["gene"][gene_count] * prob
        else:
            ...
            if gene_count == 2:
                prob = inherited_prob[mother] * inherited_prob[father] * prob
            elif gene_count == 1:
                prob = prob * ((inherited_prob[mother]*(1-inherited_prob[father]))\
                    + inherited_prob[father]*(1-inherited_prob[mother]))
            else:
                prob = (1-inherited_prob[father])*(1-inherited_prob[mother]) * prob

        prob = PROBS["trait"][gene_count][has_trait] * prob
    return prob
